    ...
    for (let layer = 0; layer <= this.layerCount; layer++) {
      for (let neuron = 0; neuron < this.weights[layer].length; neuron++) {
        for (let input = 0; input < this.weights[layer][neuron].length; input++) {
          this.layers[layer + 1][neuron] += this.layers[layer][input] * this.weights[layer][neuron][input];
        }
  ...
  private adjustWeights(): void {
    ...
    for (let layer = this.layerCount - 1; layer >= 0; layer--) {
      for (let input = 0; input < this.layers[layer + 1].length; input++) {
        let nextSum = 0;
        for (let nextNeuron = 0; nextNeuron < this.layers[layer + 2].length; nextNeuron++) {
          nextSum += this.delta[layer + 1][nextNeuron] * this.weights[layer + 1][nextNeuron][input];
        }
    ...
    for (let layer = 0; layer < this.layerCount + 1; layer++) {
      for (let neuron = 0; neuron < this.weights[layer].length; neuron++) {
        for (let input = 0; input < this.weights[layer][neuron].length; input++) {
          this.weights[layer][neuron][input] += this.alpha * this.delta[layer][neuron] * this.layers[layer][input];
        }
      }
    }
  }
