...
async function main(): Promise<void> {
  const neuralNet = new NeuralNet({
    inputSize: process.env.INPUT_SIZE ? parseInt(process.env.INPUT_SIZE) : INPUT_SIZE,
    neuronSize: process.env.NEURON_SIZE ? parseInt(process.env.NEURON_SIZE) : NEURON_SIZE,
    outputSize: process.env.OUTPUT_SIZE ? parseInt(process.env.OUTPUT_SIZE) : OUTPUT_SIZE,
    layerCount: process.env.LAYER_COUNT ? parseInt(process.env.LAYER_COUNT) : LAYER_COUNT,
  });
...
async function train(neuralNet: NeuralNet): Promise<void> {
  ...
  for (epoch = 0; epoch < EPOCHS; epoch++) {
    ...
    meanError /= meanCount;
    if (Math.abs(oldMeanError - meanError) < LEARNING_BARRIER || meanError > oldMeanError) {
      countBarrier++;
    } else {
      countBarrier = 0;
    }
